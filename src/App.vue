<template>
  <div id="app">
    ...
    <section id="work" class="section">
      <div class="container">
        <SectionHeader prefix="02." :title="appText.sectionTitles.work" />
    ...
    <section id="education" class="section">
      <div class="container">
        <SectionHeader prefix="03." :title="appText.sectionTitles.education" />
        <div class="edu-items">
    ...
    <section id="projects" class="section" data-load-section="projects">
      <div class="container">
        <SectionHeader prefix="04." :title="appText.sectionTitles.projects" />
      ...
      <div class="container">
        ...
        <template v-if="isSectionReady('interests')">
          ...
          <div v-for="cat in interestCategories" :key="cat.id" v-show="activeCat === cat.id">
            ...
            <GalleryGrid
              v-if="cat.id === 'clay'"
              :items="clayItemsForTheme"
              :aspects="galleryAspects.clay"
              :img-key-prefix="themeMode"
              placeholder-icon="🏺"
              :placeholder-text="appText.placeholders.clay"
              :more-label="appText.moreComing.clay"
              @open="(i) => openGalleryLightbox('clay', clayItemsForTheme, i)"
              @record-aspect="(k, e) => recordGalleryAspect('clay', k, e)"
              @img-error="(e, item) => handleThemeImageError(e, item.img)"
            >
              <template #sub="{ item }">{{ item.description }}</template>
            </GalleryGrid>
            ...
            <div v-if="cat.id === 'photo'">
              ...
              <GalleryGrid
                :items="activePhotoItems"
                :aspects="galleryAspects.photo"
                :key-prefix="activePhotoPlace"
                placeholder-icon="📷"
                :placeholder-text="appText.placeholders.photo"
                @open="(i) => openGalleryLightbox('photo', activePhotoItems, i)"
                @record-aspect="(k, e) => recordGalleryAspect('photo', k, e)"
              >
                <template #sub="{ item }">{{ item.location }} · {{ item.year }}</template>
              </GalleryGrid>
            </div>
            ...
            <div v-if="cat.id === 'video'">
              ...
              <VideoRowList
                v-if="['travel', 'recaps', 'tiktoks'].includes(activeVideoCat)"
                :rows="activeVideoRows"
              />
    ...
    <section id="contact" class="section">
      <div class="container">
        <SectionHeader prefix="07." :title="appText.sectionTitles.contact" />
